...
class GG_Subgrid(Gridded_Granule):
    '''
    This class handles the IS2 cloud and feature statistics 
    in the LAEA grid produced using the gridded granule IS2 
    data. 
    The difference from GG_Coarse is that the differences between
    cloud fraction over leads and over sea ice are computed per 
    granule. This method will mitigate the uncertainty due to 
    synoptic variability to some extent. 
    ...
    * Depricated, to be removed.
    ...
    def set_grid(self,
        ...
        for vn in vns:
            if vn not in ['iY','iX']:   
                ...
                # --- add diff variables to grid
                wrds = vn.split('_')
                vnpre = wrds[0]
                if len(wrds)==3:
                    vnpre = vnpre+'_'+wrds[1]
                vndiff = vnpre+'_diff'
                if vndiff not in vn_finish:
                    ods[vndiff] = (dims+['Y','X'],np.zeros(vshp))
                
        ods['n_granule'] = (['Y','X'],np.zeros(ods.lat.shape))
        # ---set nprof_diff so that the averaging for diff variables use nprof_all for scaling
        # ---Note: use deep copy here! Otherwise, double counting in ds_accumulate. 
        ods['nprof_diff'] = ods['nprof_all'].copy(deep=True)
    ...
    def add_granule(self,
        ...
        with xr.open_dataset(fn) as ds:
            nds = self.add_diff(ds)
            nds = self.apply_weighting(nds)
    ...
    def ds_normalize(self,ds: xr.Dataset, ds_type='feature') -> xr.Dataset:
        ...
        for vn in ds.data_vars:
            if vn in ['iY','iX','lat','lon','n_granule'] or 'nprof_' in vn:
                ods[vn] = ds[vn].copy(deep=True)
            else:
                wrds = vn.split('_')
                vnprof = 'nprof_'+wrds[-1]
                weight = ds[vnprof].copy(deep=True)
    ...
    def add_diff(self, ds: xr.Dataset) -> xr.Dataset:
        '''
        Add difference between lead and flow to dataset. 
        
        Parameters:
        ----------
        ds: xr.Dataste of the input data (gridded granule or on full 2D grid) 
        
        Return:
        ------
        xr.Dataset of with the lead-floe difference
        '''
        ods = xr.Dataset()
        cpp_list = ['cth','cbh','cdp']
        vn_finish = []
        for vn in ds.data_vars:
            if vn in ['iY','iX','lat','lon','n_granule'] or 'nprof_' in vn:
                ...
                vndiff = vnpre+'_diff'
                if vndiff not in vn_finish:
                    ods[vndiff] = ds[vnpre+'_lead']-ds[vnpre+'_floe']
        ods['nprof_diff'] = ds['nprof_all'].copy(deep=True)
        return ods
